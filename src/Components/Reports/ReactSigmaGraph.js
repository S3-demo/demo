...
import "./sigma.renderers.snapshot";
import "./edges.labels.curve";
import "./edges.labels.curvedArrow";
import "./edges.labels.def";
import "./settings";
...
class ReactSigmaGraph extends React.Component {
  ...
  constructor(props) {
    ...
    this.randomIdSegment = Math.random()
      .toString(36)
      .substring(7);
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    let shouldUpdate = hasStage
      ...
      : this.didDataChange(prevProps.data, this.getData());
  ...
  handleMaxSizeChange() {
    let newValue = document.getElementById(
      `rGraphSlider.${this.randomIdSegment}`
    ).value;
  ...
  getNodes() {
    ...
    if (this.state.currentFilter) {
      nodes = this.getData().nodes.filter(
        this.props.filters[this.state.currentFilter]     
      );
    } else {
  ...
  renderHeader() {
    ...
    return (
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <div>{nodes}</div>
        <div style={{ whiteSpace: "nowrap" }}>{headerText}</div>
      </div>
    );
  }
    ...
    return (
      <div>
        ...
        <input
          ...
          id={`rGraphSlider.${this.randomIdSegment}`}
          onChange={_onChange}
        />
      </div>
    );
...
const HIGHLIGHTED_EDGE_COLOR = "#e52020";
const SIZE_DEBOUNCE = 1000;
// const DEFAULT_COLOR_SCALE = d3.scale.category10();
const DEFAULT_DATA = {
  nodes: [
    { id: "a", category: "cat", name: "Garfield12" },
    { id: "b", category: "dog", name: "Pluto" },
    { id: "c", category: "dog", name: "Pluto1" }
  ],
  edges: [
    { source: "a", target: "b", label: "friend" },
    { source: "b", target: "c", label: "friend" }
  ]
};
